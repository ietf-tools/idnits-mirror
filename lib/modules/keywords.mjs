...
const INVALID_TERMS_RE = /(?![A-Za-z0-9])demultiplexor(?![A-Za-z0-9])|diffserv|(?<!\S)e[-\s]mail(?![A-Za-z0-9])|(?<![A-Za-z])internet\sdraft(?![A-Za-z])|ipsec|(?<!\S)on[-\s]line(?![A-Za-z0-9])|(?<![A-Za-z])pseudo[-\s]wire(?<![A-Za-z])|(?<![A-Za-z])public-key(?<![A-Za-z])|(?<![A-Za-z])sub-domain(?<![A-Za-z])|(?<![A-Za-z])sub-options(?<![A-Za-z])|(?<![A-Za-z])time-stamp(?<![A-Za-z])|(?<![A-Za-z])us-ascii(?<![A-Za-z])/gi
