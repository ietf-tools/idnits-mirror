...
export async function validateIntroductionSection (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      const validSectionNames = ['Introduction', 'Overview', 'Background']

      const sectionStartIndex = bodyLines.findIndex((line) =>
        validSectionNames.some((name) => {
          const trimmedLine = line.trim()
          const isSectionHeader = new RegExp(`^1\\.\\s+${name}`, 'i').test(trimmedLine)
          const isNotInTOC = !/\.+\s*\d+$/.test(trimmedLine)
          return isSectionHeader && isNotInTOC
        })
      )
      ...
      if (sectionStartIndex === -1) {
        ...
        const contentAfterSection = bodyLines.slice(sectionStartIndex + 1)

        const nextSectionIndex = contentAfterSection.findIndex((line) =>
          /^\d+\.\s/.test(line.trim())
        )

        const sectionContent = nextSectionIndex === -1
          ? contentAfterSection
          : contentAfterSection.slice(0, nextSectionIndex)

        if (!sectionContent.length || sectionContent.every((line) => !line.trim())) {
          result.push(
            new ValidationError(
...
export async function validateSecurityConsiderationsSection (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      const sectionStartIndex = bodyLines.findIndex((line) => {
        const trimmedLine = line.trim()
        const isSectionHeader = /^\d+\.\s+Security Considerations$/i.test(trimmedLine)
        const isNotInTOC = !/\.+\s*\d+$/.test(trimmedLine)
        return isSectionHeader && isNotInTOC
      })
...
export async function validateAuthorSection (doc, { mode = MODES.NORMAL } = {}) {
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      const validHeaders = [
        /^[0-9a-z.]* *((author|editor)(['’]s|s['’]) +(and +)?)+address(es)?$/i,
        /^[0-9a-z.]* *author information$/i,
        /^[0-9a-z.]* *(author|editor)(['’]s|s['’]) contact information$/i,
        /^[0-9a-z.]* *contact information$/i,
        /^[0-9a-z.]* *(author|editor)s?:?$/i
      ]

      const misspelledHeaders = [
        /^[ \t0-9.]*(Author|Editor)([^\047]s|s[^\047]) [Aa]ddress(es)?$/i,
        /^[ \t0-9.]*(Authors?|Editors?) +[Aa]ddress(es)?$/i
      ]

      let hasValidHeader = false
      let hasMisspelledHeader = false
      let foundHeaderLine = ''

      for (const line of bodyLines) {
        const trimmedLine = line.trim()

        const isInTOC = /\.+\s*\d+$/.test(trimmedLine)
        if (isInTOC) continue

        if (validHeaders.some((regex) => regex.test(trimmedLine))) {
          hasValidHeader = true
          foundHeaderLine = trimmedLine
          break
        }

        if (misspelledHeaders.some((regex) => regex.test(trimmedLine))) {
          hasMisspelledHeader = true
          foundHeaderLine = trimmedLine
        }
      }
      ...
      if (hasValidHeader) {
        break
      } else if (hasMisspelledHeader) {
        result.push(
          new ValidationWarning(
            'MISSPELLED_AUTHOR_SECTION',
            `The author section title "${foundHeaderLine}" appears misspelled or incorrectly formatted.`,
            {
