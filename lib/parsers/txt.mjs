...
const KEYWORDS_PATTERN = /\b(MUST|MUST NOT|SHALL|SHALL NOT|SHOULD|SHOULD NOT|RECOMMENDED|NOT RECOMMENDED|MAY|OPTIONAL)\b/g
...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    for (const line of rawText.split('\n')) {
      ...
      const invalidMatches = [...trimmedLine.matchAll(INVALID_COMBINATIONS_PATTERN)]
      invalidMatches.forEach(match => {
        data.possibleIssues.misspeled2119Keywords.push({ invalidKeyword: match[0], line: lineIdx })
      })
